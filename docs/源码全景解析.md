...
- **风险1：Claude Code token 统计存在偏差**
  描述：两端点的 `message_start.input_tokens` 均来自本地估算，偏差可达 10-20%；流末尾 `message_delta` 会做校准，但 `/context` 显示依赖该终值
  规避：核对日志中的 `[input_tokens] 本地化: estimated=N final=M`，偏差异常时检查 prompt cache 命中情况
