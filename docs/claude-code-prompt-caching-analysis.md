...
缓存不是永久有效的。每次命中会刷新其 TTL（Time To Live，生存时间）；如果缓存长时间未被访问，则自动过期失效，下次请求需要重新全量计算并写入缓存。

这正是 Claude Code 团队**像监控系统可用性一样监控缓存命中率**的原因——如果命中率骤降，说明某处变更破坏了前缀稳定性，或用户会话中断时间过长导致缓存批量过期，成本会显著上升。
