...
// 事件3：使用量（stream末尾）
{"contextUsageEvent": {"contextUsagePercentage": 5.2}}
```
...
3. `contextUsageEvent.contextUsagePercentage` → 仅用于 ≥100% 时设 `stop_reason` 与用量诊断（不参与 `input_tokens` 计算）
...
EventStreamDecoder (kiro/parser/decoder.rs) ← 解码 AWS 二进制帧
  ...
  │
  ▼
StreamContext (anthropic/stream.rs)          ← 转换为 SSE
  │  /v1 与 /cc/v1 共用：实时转换，每帧立即发给客户端
  │  差异仅在 /cc/v1 额外带 300s 全局 deadline（上游挂起保护）
  │
  ▼
...
- **`/cc/v1` 上游挂起保护**：与 `/v1` 同为实时转发，额外带 300s 全局 deadline，撞线发 `overloaded_error` 后终止
...
├── token.rs                   # Token 计数（tiktoken cl100k_base BPE 精确计算）
...
sequenceDiagram
    ...
    AX->>AX: StreamContext 逐帧实时转换（/cc/v1 额外挂 300s deadline）
    AX-->>CC: text/event-stream（Anthropic SSE，input_tokens 末尾 message_delta 校准）
```
...
### 解析二：/cc/v1 与 /v1 的唯一差异 — 300s 全局 deadline

**文件位置：** `src/anthropic/handlers.rs`

两个端点的流式路径已完全统一，共用 `handle_stream_request` + `create_sse_stream`。唯一差异是 `/cc/v1` 多传一个 `Some(Duration::from_secs(300))`：

```
/v1：
  Kiro frame → 立即转换为 SSE → 发给客户端（无全局超时）

/cc/v1：
  Kiro frame → 立即转换为 SSE → 发给客户端
  + tokio::select! 中额外挂一个 deadline 分支
    撞线 → 发 overloaded_error 事件 → 终止流（已流出的内容保持有效）
```

实现靠 `wait_deadline(deadline: Option<Instant>)`：`None` 时走 `std::future::pending()` 永不就绪，使 `select!` 的该分支等价于不存在 —— 故 `/v1` 行为零变化。

**input_tokens 怎么给**：两端点一致 —— `message_start` 与流末尾 `message_delta` 的 usage **总量恒为本地估算值**经 `cap_input_tokens` 上限截断（`stream.rs:1430-1432`），不采纳 `meteringEvent` 真值。变化的只是这个总量里划给缓存的部分：优先 `meteringEvent` 透传，其次前缀估算，最后按 `PromptCacheUsage` 比例模拟，拆出 `cache_read_input_tokens` / `cache_creation_input_tokens`，剩余非缓存部分才是对外报告的 `usage.input_tokens`。`context_input_tokens` 字段已无任何赋值点（恒为 `None`，见 `stream.rs:1429`），`contextUsageEvent` 不再参与 `input_tokens` 计算。
...
① message_start（流开始，只出现一次）
   {
     ...
     "message": {
       ...
       "stop_sequence": null,
       "usage": {
         "input_tokens": 150,                    // 两端点一致：本地估算，末尾 message_delta 校准
         "output_tokens": 1,
...
└── context_usage_percentage: f64
      上下文窗口使用百分比（0-100）
      → 写入用量记录作诊断字段（不参与 input_tokens 计算）
...
### 难点 7：Token 计算——从四分类加权公式到 tiktoken 精确 BPE

**难在哪里**：Kiro API 不会像 Anthropic 官方 API 那样在响应里带精确的 input token usage，代理必须自己估算。旧实现用"字符分类加权"公式（字母 /4.5、数字 /2.0、符号和 CJK /1.5），本质是拍脑袋的经验公式，与真实 BPE 分词结果偏差可达 20%+，中文/符号密集场景失真尤其明显。

**心智模型**：就像用"体重估算身高"——旧公式靠字符统计做代理指标，tiktoken 则是直接称重（执行与 OpenAI/Anthropic 生产环境同源的 cl100k_base BPE 分词算法），不再是猜测。

**核心实现**（`src/token.rs`）：
```rust
fn get_bpe() -> &'static CoreBPE {
    // 全局单例，首次调用初始化(~50ms)；编码表通过 include_str! 编译期内嵌进二进制，无运行时下载风险
...
| 函数 | 用途 | 覆盖范围 |
|---|---|---|
| `count_tokens(text)` | 最底层原子计数，对单段文本做 BPE 编码 | 单个字符串 |
| `count_content_block(block)` | 按 content block 类型分派：text 取 text 字段；tool_use 取 name+input JSON；tool_result 递归取 content | 单个消息块 |
| `count_all_tokens_local()` | 汇总 system + 全部 messages + tools 的总 token 数 | 整个请求（纯本地估算） |
| `count_all_tokens()` | 优先调用外部 count_tokens API，失败回退 `count_all_tokens_local()` | 整个请求（含远程优先级） |
| `count_prefix_tokens()` | 仅统计 system+tools+除最后一条 user 外的历史消息（估算 cache 前缀） | 请求前缀（不含末轮 user） |

调用入口在 `src/anthropic/handlers.rs`（`/v1` 与 `/cc/v1` 两处同构调用，约 850/857/862 行与 1760/1767/1772 行）：请求进来后先算出 `prefix_estimated_tokens`（前缀部分）与本地整体估算值，供 `stream.rs` 的 `derive_report_cache_usage()` 做 usage 三段式拆分。

**cache usage 三级降级**（`src/anthropic/stream.rs` `derive_report_cache_usage()`）：
```
1. Kiro meteringEvent 真实用量（最高优先级，若存在）
2. prefix_estimated_tokens 前缀估算（用 count_prefix_tokens 结果近似 cache_read）
3. prompt_cache_usage 比例模拟（全部降级失败时的兜底比例分配）
```

**踩坑记录**：替换前 `count_tokens("")` 靠 `.max(1)` 兜底返回 `1`；替换后改为直接返回 `0`，把"至少 1"的兜底职责下放给调用方（`count_all_tokens_local`/`estimate_output_tokens` 各自 `.max(1)`），避免在最底层函数里混入调用方语义。测试断言也从旧公式的估算区间改为 tiktoken 真实值（如 `count_tokens("你好世界") == 5`）。

**与 auto-compact 阈值的关联（潜在风险）**：`stream.rs` 中的 `CLIENT_TOKEN_DISPLAY_SCALE = 0.6657` 是配合**旧字符加权公式**实测校准出来的经验系数，用于把代理内部真实 token 数缩放成 Claude Code 客户端在其固定假设的 200K 窗口下、能正确触发 82% auto-compact 阈值（显示值 164,000，反算真实阈值 ≈246,357）所需要看到的显示值。**tiktoken 替换后单条文本的计数口径已经变化，`0.6657` 这个系数理论上需要重新实测校准**，否则可能导致客户端提前或滞后触发上下文压缩。

---

### 难点 8：additionalModelRequestFields 的"模型代际 × 字段"约束矩阵

**难在哪里**：`additionalModelRequestFields`（`converter.rs:1528 build_additional_model_request_fields()`）承载 thinking / output_config / max_tokens 三个子字段，但 Kiro 后端对"哪些模型接受哪些字段、取值范围是多少"按模型代际区别对待，且拒绝时统一报 400，错误信息不指明是哪个字段越界。规则只能逐模型代际实测，任何一行缺席都可能 400。

**实测约束矩阵**：

| 子字段 | 规则 | 实测/代码依据 |
|---|---|---|
| `thinking` | 仅请求携带 thinking 配置时写入；`enabled` 与 `adaptive` 统一下发为 `{"type":"adaptive"}`，显式 `disabled` 原样写入 | converter.rs 现行实现 |
| `output_config.effort` | 未指定时默认 `high` | 同上 |
| `max_tokens` 下限 | schema 强制 `minimum = 1024`，对**全部 Claude 代际**生效（claude-sonnet-4-6 传 200 同样 400："must have a minimum value of 1024.0"），并非只有 128K 上限档才有此限制 | d669fb6 回归测试（converter.rs:3975） |
| `max_tokens` 上限 | opus-4.7 / 4.8 / 5 = **128K**（1M 窗口代际）；其余（含 sonnet-5、haiku）= **64K**（`converter.rs:1507 model_max_output_tokens()`） | d02cd86 |
| 整体拒绝 | gpt-5.6-* 全系列不认识 max_tokens、output_config 子字段（400 REQUEST_BODY_INVALID），需整体跳过、返回 `None` | c4981f2（converter.rs:3610 测试） |

**修复**：GPT 系（`is_gpt_model()`）直接返回 `None`；其余模型逐字段填充，`max_tokens` 走 `min(上限).max(1024)` 双向夹紧。调用链：`converter.rs:940` 构建 → `handlers.rs:821`（`/v1`）与 `handlers.rs:1778`（`/cc/v1`）同构使用。

**⚠️ 现状核对（注释与代码不一致）**：早期实测（df7f471）claude-sonnet-4.5 / claude-opus-4.5 / claude-haiku-4.5 整体拒绝该字段，当时加入了 `ends_with("4.5")` 跳过逻辑；但 3996ba8 重构时移除了该跳过（仅保留 GPT 判断），后续 d669fb6 的回归测试又明确断言 haiku-4.5 **应当**构建字段——两代实测结论冲突。当前行为以代码为准：**4.5 代际不再跳过**。`build_additional_model_request_fields` 上方的文档注释仍声称 4.5 代际需跳过，属遗留未更新；4.5 代际当前到底接不接受该字段，需用真实 Kiro 流量复测后统一"注释、跳过逻辑、测试断言"三者口径。
...
- **问题**：Claude Code 会话动辄跑满上下文窗口，上游一旦挂起会让请求永不结束；curl / Open WebUI 这类客户端则不希望被加上人为超时
- **方案**：两端点共用同一条实时流式路径，`/cc/v1` 额外带 300s 全局 deadline（撞线发 `overloaded_error` 后终止）
...
- **风险1：Claude Code token 统计存在偏差**
  描述：两端点的 `message_start.input_tokens` 均来自本地 tiktoken cl100k_base BPE 计算（见"难点突破 → 难点 7"），已比旧字符加权公式精确得多，但仍非 Anthropic 官方 tokenizer；且 `CLIENT_TOKEN_DISPLAY_SCALE`（0.6657）是基于旧公式校准的经验系数，替换后可能需要重新校准；流末尾 `message_delta` 会做校准，但 `/context` 显示依赖该终值
  规避：核对日志中的 `[input_tokens] 本地化: estimated=N final=M`，偏差异常时检查 prompt cache 命中情况及 `CLIENT_TOKEN_DISPLAY_SCALE` 是否需要重新实测校准
