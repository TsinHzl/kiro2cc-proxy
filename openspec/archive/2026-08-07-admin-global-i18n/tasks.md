...
- [x] 设置页面新增语言切换控件，切换后 `localStorage` 写入 `admin-lang`，刷新页面后语言保持
- [x] 无 `localStorage` 记录时默认中文渲染
- [x] 切换语言后，任务 3-9 覆盖的全部页面文案实时切换为对应语言，无遗漏硬编码中文
- [x] 更新日志页面不再显示页面内独立语言切换按钮，且跟随全局语言渲染对应字段
- [x] 设置页面视觉呈现为分组列表（灰色分组标题 + 圆角容器 + 行分隔线），认证密钥与负载均衡原有交互功能不变
- [x] `cargo fmt`/`cargo clippy`/`tsc`/`npm run build` 全部通过
