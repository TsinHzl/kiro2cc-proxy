...
mod tests {
    ...
    async fn test_get_changelog_returns_sorted_list_with_single_latest() {
        ...
        // 至多一条 latest；若 current 版本在列表里，首条就是它且 is_latest=true
        assert!(parsed.data.iter().filter(|n| n.is_latest).count() <= 1);
        ...
        if parsed.data.iter().any(|n| n.version == current) {
            assert_eq!(parsed.data[0].version, current);
