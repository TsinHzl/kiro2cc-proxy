...
const FAMILY_AUTO = '@@auto'
const FAMILY_OTHER = '@@other'

/** 新命名：族名在前（claude-opus-4-1-…），主版本即一代 */
const MODERN_CLAUDE = /^claude-(?:opus|sonnet|haiku)-(\d+)/
/** 旧命名：版本在前（claude-3-7-sonnet-…），主次版本合起来才是一代 */
const LEGACY_CLAUDE = /^claude-(\d+)(?:-(\d+))?-/
 ...
 */
function modelFamily(model: ModelItem): string {
...
function groupByFamily(models: ModelItem[]): FamilyGroup[] {
...
function rateRange(models: ModelItem[]): RateRange | null {
...
function formatRateRange(range: RateRange | null): string {
